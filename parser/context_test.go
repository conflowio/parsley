...
func TestRegisterResultShouldSaveResultForPosition(t *testing.T) {
	...
	c.RegisterResults(c.GetParserIndex("p1"), 2, results, data.NewIntMap())

	actual, ok := c.GetResults(c.GetParserIndex("p1"), 2, data.NewIntMap())
...
func TestRegisterResultShouldReturnNilResult(t *testing.T) {
	c := parser.NewContext()
	c.RegisterResults(c.GetParserIndex("p1"), 2, nil, data.NewIntMap())
	results, ok := c.GetResults(c.GetParserIndex("p1"), 2, data.NewIntMap())
...
func TestRegisterResultShouldHandleMultipleParsers(t *testing.T) {
	...
	node := ast.NewTerminalNode("t", reader.NewPosition(0, 1, 2), nil)
	results := parser.NewParserResult(nil, parser.NewResult(node, nil))
	c.RegisterResults(c.GetParserIndex("p1"), 1, results, data.NewIntMap())
	c.RegisterResults(c.GetParserIndex("p2"), 2, nil, data.NewIntMap())

	actual, ok := c.GetResults(c.GetParserIndex("p1"), 1, data.NewIntMap())
	assert.Equal(t, results, actual)
	assert.True(t, ok)

	results, ok = c.GetResults(c.GetParserIndex("p2"), 2, data.NewIntMap())
	assert.Nil(t, results)
	assert.True(t, ok)
}
...
func TestGetResultsShouldNotReturnCurtailedResult(t *testing.T) {
	...
	c.RegisterResults(c.GetParserIndex("p1"), 1, parser.NewParserResult(ctx), data.NewIntMap())

	ctx = data.NewIntMap()
	ctx[c.GetParserIndex("p1")] = 1
	ctx[c.GetParserIndex("p2")] = 1
	ctx[c.GetParserIndex("p3")] = 1
	results, found := c.GetResults(c.GetParserIndex("p1"), 1, data.NewIntMap())
	assert.Nil(t, results)
...
func TestGetResultsShouldReturnCurtailedResult(t *testing.T) {
	...
	c.RegisterResults(c.GetParserIndex("p1"), 1, cResults, data.NewIntMap())
	c.RegisterResults(c.GetParserIndex("p2"), 1, cResults, data.NewIntMap())
	...
	var found bool
	ctx[c.GetParserIndex("p3")] = 1
	_, found = c.GetResults(c.GetParserIndex("p3"), 1, data.NewIntMap())
	assert.False(t, found)

	ctx[c.GetParserIndex("p1")] = 1
	_, found = c.GetResults(c.GetParserIndex("p1"), 1, data.NewIntMap())
	assert.False(t, found)

	ctx[c.GetParserIndex("p2")] = 1
	_, found = c.GetResults(c.GetParserIndex("p2"), 1, data.NewIntMap())
	assert.False(t, found)
